...
abstract class Table
{
    ...
    {
        return \IAWP_SCOPED\iawp_blade()->run('tables.column-picker-modal', ['columns' => $this->get_columns()]);
    }
    public function group_picker_html() : ?string
    {
        ...
        return \IAWP_SCOPED\iawp_blade()->run('tables.group-select', ['options' => $this->groups()->buttons(), 'group' => $this->group()]);
    }
    public function get_table_toolbar_markup()
    {
        return \IAWP_SCOPED\iawp_blade()->run('tables.table-toolbar', ['all_columns' => $this->get_columns(), 'group_html' => $this->group_picker_html()]);
    }
    public function get_table_markup(string $sort_column, string $sort_direction)
    {
        return \IAWP_SCOPED\iawp_blade()->run('tables.table', ['table' => $this, 'table_name' => $this->table_name(), 'all_columns' => $this->get_columns(), 'visible_column_count' => $this->visible_column_count(), 'number_of_shown_rows' => 0, 'rows' => [], 'render_skeleton' => \true, 'page_size' => \IAWP_SCOPED\iawp()->pagination_page_size(), 'sort_column' => $sort_column, 'sort_direction' => $sort_direction, 'has_campaigns' => Campaign_Builder::has_campaigns()]);
    }
    ...
    public function output_toolbar()
    {
        ...
        <div class="download-options-parent" data-controller="modal">
            <div class="modal-parent downloads">
                <button data-modal-target="modalButton" data-action="click->modal#toggleModal" class="download-options">
                    <?php 
        ...
        ?></div>
                        <button class="iawp-button" data-report-target="exportCSV" data-action="report#exportCSV">
        ...
        ?>
                        ...
                        <button class="iawp-button" data-report-target="exportPDF" data-action="report#exportPDF" disabled="disabled">
    ...
    {
        if ($just_rows) {
            return \IAWP_SCOPED\iawp_blade()->run('tables.rows', ['table' => $this, 'table_name' => $this->table_name(), 'all_columns' => $this->get_columns(), 'visible_column_count' => $this->visible_column_count(), 'number_of_shown_rows' => \count($rows), 'rows' => $rows, 'render_skeleton' => \false, 'page_size' => \IAWP_SCOPED\iawp()->pagination_page_size(), 'sort_column' => $sort_column, 'sort_direction' => $sort_direction, 'has_campaigns' => Campaign_Builder::has_campaigns()]);
        }
        return \IAWP_SCOPED\iawp_blade()->run('tables.table', ['table' => $this, 'table_name' => $this->table_name(), 'all_columns' => $this->get_columns(), 'visible_column_count' => $this->visible_column_count(), 'number_of_shown_rows' => \count($rows), 'rows' => $rows, 'render_skeleton' => \false, 'page_size' => \IAWP_SCOPED\iawp()->pagination_page_size(), 'sort_column' => $sort_column, 'sort_direction' => $sort_direction, 'has_campaigns' => Campaign_Builder::has_campaigns()]);
    }
