...
class ExtMongoDBCache extends CacheProvider
{
    ...
    {
        try {
            $this->collection->updateOne(['_id' => $id], ['$set' => [MongoDBCache::EXPIRATION_FIELD => $lifeTime > 0 ? new UTCDateTime((time() + $lifeTime) * 1000) : null, MongoDBCache::DATA_FIELD => new Binary(serialize($data), Binary::TYPE_GENERIC)]], ['upsert' => \true]);
        } catch (Exception $e) {
            return \false;
        }
        return \true;
    ...
    protected function doGetStats()
    {
        ...
        return [Cache::STATS_HITS => null, Cache::STATS_MISSES => null, Cache::STATS_UPTIME => $uptime, Cache::STATS_MEMORY_USAGE => $memoryUsage, Cache::STATS_MEMORY_AVAILABLE => null];
    }
    /**
    ...
    {
        return isset($document[MongoDBCache::EXPIRATION_FIELD]) && $document[MongoDBCache::EXPIRATION_FIELD] instanceof UTCDateTime && $document[MongoDBCache::EXPIRATION_FIELD]->toDateTime() < new DateTime();
    }
