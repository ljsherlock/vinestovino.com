...
class LegacyMongoDBCache extends CacheProvider
{
    ...
    {
        try {
            $result = $this->collection->update(['_id' => $id], ['$set' => [MongoDBCache::EXPIRATION_FIELD => $lifeTime > 0 ? new MongoDate(time() + $lifeTime) : null, MongoDBCache::DATA_FIELD => new MongoBinData(serialize($data), MongoBinData::BYTE_ARRAY)]], ['upsert' => \true, 'multiple' => \false]);
    ...
    protected function doGetStats()
    {
        ...
        return [Cache::STATS_HITS => null, Cache::STATS_MISSES => null, Cache::STATS_UPTIME => $serverStatus['uptime'] ?? null, Cache::STATS_MEMORY_USAGE => $collStats['size'] ?? null, Cache::STATS_MEMORY_AVAILABLE => null];
    }
    /**
    ...
    {
        return isset($document[MongoDBCache::EXPIRATION_FIELD]) && $document[MongoDBCache::EXPIRATION_FIELD] instanceof MongoDate && $document[MongoDBCache::EXPIRATION_FIELD]->sec < time();
    }
    private function createExpirationIndex() : void
    {
        ...
        $this->collection->createIndex([MongoDBCache::EXPIRATION_FIELD => 1], ['background' => \true, 'expireAfterSeconds' => 0]);
    }
}
