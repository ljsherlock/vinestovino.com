...
class Helper {
	...
	public function event_registration_deadline($args) {
		...
		$settings                = \Etn\Utils\Helper::get_settings();
		$selected_expiry_point   = $settings['expiry_point'] ?? 'end';
		$selected_expiry_time    = $settings['expiry_time'] ?? 'start';
		$event_expire_start_date = get_post_meta( $single_event_id, "etn_start_date", true ) ?: "";
		$event_expire_start_time = get_post_meta( $single_event_id, "etn_start_time", true ) ?: "";
		$event_expire_end_time   = get_post_meta( $single_event_id, "etn_end_time", true ) ?: $event_expire_start_time;
		$etn_deadline            = get_post_meta( $single_event_id, 'etn_registration_deadline', true );
		$event_timezone          = get_post_meta( $single_event_id, "event_timezone", true ) ?: "UTC+0"; // Request from theme team for default value
	
		$event_expire_date = ( $selected_expiry_point == "start" ) ? $event_expire_start_date :
			( get_post_meta( $single_event_id, "etn_end_date", true ) ?: $event_expire_start_date );
	
		$event_expire_date_time = ($etn_deadline !== '') ? $etn_deadline :
			( $event_expire_date . " " . ( ( $selected_expiry_time == "start" ) ? $event_expire_start_time : $event_expire_end_time ) );
	
		$now_dt = gmdate( "Y-m-d H:i" );
	
		if ( strpos( $event_timezone, 'UTC+' ) !== false || strpos( $event_timezone, 'UTC-' ) !== false ) {
			$time_cal = \Etn\Utils\Helper::time_calculation( $event_timezone );
			$dt = gmdate( "Y-m-d H:i", strtotime( "$event_expire_date_time" ) );
			$now_dt = gmdate( "Y-m-d H:i", $time_cal );
		} else {
			date_default_timezone_set( $event_timezone );
			$dt = gmdate( "Y-m-d H:i", strtotime( "$event_expire_date_time $event_timezone" ) );
		}
